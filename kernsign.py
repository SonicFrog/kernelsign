...
# Default kernel and initrd format for gentoo
...
# Non GPG kernel signature checking
def non_gpg_check(kernel, initrd, algo='sha256'):
    h = hashlib.new(algo)
...
def non_gpg(kernel, initrd, homedir, alg='sha256'):
    if alg not in hashlib.algorithms_available:
        raise ValueError("Unable to find " + alg)
    h = hashlib.new(alg)
...
def check_sigs(gpg, kernel, initrd, homedir):
    try:
        ...
        print(io)
        Logger.error("Unable to open signatures! Please update signatures " +
                     "before checking!")
        return False
...
if __name__ == '__main__':
    ...
    parser.add_argument("-q", "--quiet", help="Put the program in quiet " +
                        " mode (log nothing)", actions="store_true",
                        default=False)
    actions.add_argument("-c", "--check", help="Checks the kernel and " +
                         "initrd for validity against their signatures",
                         action="store_true", default=False)
    actions.add_argument("-g", "--generate-key", help="Generates a new " +
                         "key for signing the kernel and initramfs",
                         action="store_true", default=False)
    actions.add_argument("-u", "--update-sigs", help="Updates the " +
                         "signature for the given kernel and initramdisk",
                         action="store_true", default=False)
    parser.add_argument("-d", "--homedir", help="The path to the GnuPG " +
                        "homedir to use for verifying/signing",
                        default=homedir)
    parser.add_argument("-s", "--key-size", help="Specify the key size " +
                        "to generate for signing", type=int, default=4096)
    parser.add_argument("-t", "--key-type", help="Specify the type of key " +
                        "to generate for signing", type=str, default="RSA")
    ...
    try:
        if arguments.generate_key:
            ...
            if update_sigs(gpg, arguments.kernel, arguments.initrd,
                           arguments.homedir):
            ...
            Logger.info("Checking signature for kernel " + arguments.kernel +
                        "...")
            try:
                check_sigs(gpg, arguments.kernel, arguments.initrd,
                           arguments.homedir)
            except ValueError:
